...
if __name__ == "__main__":
    ...
    reversed_paths = reversed([p for p in backups.glob("*")])
    paths_to_remove = []
    ...
    for curr_path in reversed_paths:
        ...
        if last_path and str(last_path.stem)[:-15] == str(curr_path.stem)[:-15]:
            ...
            elif timediff > delta_min + delta_max[i - 1]:
                delta_min += delta_max[i - 1]
                if removed_something:
                    paths_to_remove.pop(-1)
                    removed_something = False
            # remove other paths greater than the minimum
            elif timediff > delta_min:
                paths_to_remove.append(last_path)
            ...
            dlst = str(curr_path.stem)[:-15].split("-")
            ref_date = datetime(int(dlst[0]), int(dlst[1]), int(dlst[2]), int(dlst[3]), int([4][:2]), int(dlst[4][2:]))
            last_path = curr_path
